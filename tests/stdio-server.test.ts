...
describe("grok-imagine stdio MCP server", () => {
  ...
  it("exposes the model-line tools over the real stdio transport", async () => {
    ...
    client = new Client({ name: "grok-imagine-mcp-test", version: "0.1.0" });
    ...
    expect(names).toEqual(["check_pricing","edit_image","extend_video","get_task","image_to_video","text_to_image","text_to_video","upscale_image"]);
    ...
    for (const model of ["grok-imagine-edit-image","grok-imagine-image-to-video","grok-imagine-text-to-image","grok-imagine-text-to-video"]) {
